...
void InitGraphics()
{
    ...
    Init07TextureBufferAndFillFromBmpFile("IMG_0937.bmp", &MyPuppyTexture);
...
Init03PhysicalDeviceAndGetQueueFamilyProperties()
{
    ...
    for (unsigned int i = 0; i < PhysicalDeviceCount; i++)
    {
        ...
        if (vpdp.deviceType == VK_PHYSICAL_DEVICE_TYPE_VIRTUAL_GPU)
            fprintf(FpDebug, " (Virtual GPU)\n");
        if (vpdp.deviceType == VK_PHYSICAL_DEVICE_TYPE_CPU)
            fprintf(FpDebug, " (CPU)\n");
...
#ifdef COMMENT
    All of these VkPhysicalDeviceFeatures are VkBool32s : robustBufferAccess;
    fullDrawIndexUint32;
    imageCubeArray;
    independentBlend;
    geometryShader;
    ...
    for (unsigned int i = 0; i < vpdmp.memoryTypeCount; i++)
    {
        ...
        if ((vmpf & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT) != 0)
            fprintf(FpDebug, " DeviceLocal");
        if ((vmpf & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) != 0)
            fprintf(FpDebug, " HostVisible");
        if ((vmpf & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT) != 0)
            fprintf(FpDebug, " HostCoherent");
        if ((vmpf & VK_MEMORY_PROPERTY_HOST_CACHED_BIT) != 0)
            fprintf(FpDebug, " HostCached");
        if ((vmpf & VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT) != 0)
    ...
    for (unsigned int i = 0; i < vpdmp.memoryHeapCount; i++)
    {
        ...
        if ((vmh.flags & VK_MEMORY_HEAP_DEVICE_LOCAL_BIT) != 0)
            fprintf(FpDebug, " DeviceLocal");
        if ((vmh.flags & VK_MEMORY_HEAP_MULTI_INSTANCE_BIT) != 0)
            fprintf(FpDebug, " MultiInstance");
        fprintf(FpDebug, "\n");
    }
...
Init08Swapchain()
{
    ...
    VK_PRESENT_MODE_MAILBOX_KHR = 1, < -- -- -the 1080tis can handle this VK_PRESENT_MODE_FIFO_KHR = 2, < -- -- -the 1080tis can handle this VK_PRESENT_MODE_FIFO_RELAXED_KHR = 3, < -- -- -the 1080tis can handle this VK_PRESENT_MODE_SHARED_DEMAND_REFRESH_KHR = 1000111000,
...
VkDescriptorType:
    ...
    vdpci.flags = 0;
#ifdef CHOICES
    0 VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT
#endif
        vdpci.maxSets = 4;
...
Init14GraphicsVertexFragmentPipeline(VkShaderModule vertexShader, VkShaderModule fragmentShader, VkPrimitiveTopology topology, OUT VkPipeline *pGraphicsPipeline)
{
    ...
    vpcbas.colorWriteMask = VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT | VK_COLOR_COMPONENT_A_BIT;
    vpcbas.blendEnable = VK_FALSE;
...
vsme[0].constantId = << the constant_id in the layout line, uint32_t >> vsme[0].offset = << how far into the raw data this constant is, bytes >> vsme[0].size = << size of this SC in the raw data >>
...
VK_EVENT_SET:
    ...
    RenderScene()
    {
        ...
        if (Verbose && NumRenders <= 2)
            fprintf(FpDebug, "nextImageIndex = %d\n", nextImageIndex);
    ...
    {
        if (Verbose)
            fprintf(FpDebug, "Mouse button = %d; Action = %d\n", button, action);
